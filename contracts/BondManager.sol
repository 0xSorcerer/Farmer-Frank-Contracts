...
contract fNFTBond is ERC721, Ownable {
    ...
    uint256 internal GLOBAL_PRECISION = 10**18;
    uint256 internal WEIGHT_PRECISION = 100;
    uint16 internal constant MAX_BOND_LEVELS = 10;

    //HISTORIC BOND LEVELS
    mapping(bytes4 => BondLevel) private bondLevels;
    bytes4[] private activeBondLevels;
    uint8 public totalBondLevels;
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        ...
        _addBondLevelAtIndex("Level I", 10, 100, totalBondLevels);
        _addBondLevelAtIndex("Level II", 100, 105, totalBondLevels);
        _addBondLevelAtIndex("Level III", 1000, 110, totalBondLevels);
        _addBondLevelAtIndex("Level IV", 5000, 115, totalBondLevels);
    }
    ...
    //Encoded level is simply a unique identifier for each level. Since the contract is designed to support
    //Addition and deletion of Bond levels at will, it is fundamental that even if a level gets deleted, 
    //Every bond of that level 
    function _addBondLevelAtIndex(
        string memory _name,
        uint16 _basePrice,
        uint16 _weight,
        uint16 _index
    ) public onlyOwner returns (bytes4) {
        require(!(totalBondLevels >= MAX_BOND_LEVELS));
        require(_index <= totalBondLevels);
    ...
    /// @dev When removing a bond level the mapping is shifted. We disregard unbounded gas cost possible error
    /// as the bondLevels mapping is designed to store a "concise" amount of levels. This mapping will never
    /// need to store more than 10 levels.

    function _deactivateBondLevel(bytes4 levelID) public onlyOwner {
        require(bondLevels[levelID].active == true);
    ...
    function _activateBondLevel(bytes4 levelID, uint16 _index) public onlyOwner {
        require(!(totalBondLevels >= MAX_BOND_LEVELS));
        require(bondLevels[levelID].active == false);
    ...
    function mintBonds(
        ...
        require(bondLevels[levelID].active);
        require(
            _amount <= 20,
            "fNFT Bond: You can't mint more than 20 bonds in a single transaction"
        );
    ...
    ) external onlyIfExists(_bondID) /*onlyOwner*/
    {
        ...
        bond.unweightedShares = SafeMath.add(
            bond.unweightedShares,
            issuedShares
        );
    ...
    function getBondLevel(bytes4 _encodedLevel) public view returns (BondLevel memory level) {
        level = bondLevels[_encodedLevel];
    }
...
contract BondManager is Ownable, BondDiscountable {
    ...
    function addBondLevel(string memory _name, uint16 _basePrice, uint16 _weight) external onlyOwner returns (bytes4) {
        return bond._addBondLevelAtIndex(_name, _basePrice, _weight, bond.totalBondLevels());
    }
